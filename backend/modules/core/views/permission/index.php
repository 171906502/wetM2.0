...
function myCallback(json) {
    ...
    if(json.statusCode == 200){
		...
		// 增加修改 Ztree 节点
    	var treeId = "core-permission-index-ztree";
...
function addHoverDom(treeId, treeNode) {
	...
	$("#diyBtn_add_"+treeNode.id).bind("click", function(){
		 $(this).dialog({id:'core-permission-view', url:'<?= Url::toRoute(['view']) ?>&tId='+treeNode.tId+'&parent='+treeNode.id+'&name=', title:'增加节点'});
	});
	$("#diyBtn_edit_"+treeNode.id).bind("click", function(){
		 $(this).dialog({id:'core-permission-view', url:'<?= Url::toRoute(['view']) ?>&tId='+treeNode.tId+'&name='+treeNode.id, title:'修改节点'});
	});
...
<div class="bjui-pageHeader">
    <a href="<?= Url::toRoute(['view','parent'=>Yii::$app->request->get('name'), 'name'=>'']) ?>" class="btn btn-green" data-toggle="dialog" data-id="core-permission-view" data-title="增加主节点" data-reload-warn="本页已有打开的内容，确定将刷新本页内容，是否继续？">增加主节点</a>
</div>
<div class="bjui-pageContent">
    <div style="float: left; width: 250px;">
        <ul id="core-permission-index-ztree" class="ztree" data-toggle="ztree" data-options="{
            expandAll: false,
            onClick: 'ZtreeClick',
    ...
    <div style="margin-left: 260px; height: 99.9%; overflow: auto;">
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li class="active"><a href="#permission-help" role="tab" data-toggle="tab">帮助</a></li>
        </ul>
        <!-- 选项卡窗格 -->
        <div class="tab-content">
            <div id="permission-help" class="tab-pane fade active in">
                <h5>1 增加主节点</h5>
                <ol>
                    <li>点击左上角的
